...
from AI.evaluation.interviewer.followup_generator import generate_followup_question
...
def evaluate_answer(request: EvaluationRequest) -> dict[str, Any]:
    ...
    follow_up = generate_followup_question(
        ...
        followup_strategy={
            "adaptive_gap": classification.get(
                "primary_adaptive_gap", "concept coverage"
            ),
            "objective": "clarify the candidate's reasoning",
            "focus": classification.get(
                "adaptive_classifications", []
            ),
            "instruction": "Ask the next most useful interview question based on the evidence.",
        },
    )

    if not follow_up or not follow_up.strip():
        raise RuntimeError("The evaluation layer did not produce a follow-up question")
...
def evaluate_turn(request: EvaluationRequest) -> dict[str, Any]:
    try:
        return {"success": True, "data": evaluate_answer(request)}
